...
apiRouter.get("/:id/extrato", async (req, res) => {
  const id = req.params.id;
  const query2 = await db.query({
    text: "SELECT c.limite, c.saldo, t.realizada_em, t.descricao, t.tipo FROM clientes c JOIN transacoes t ON c.id = t.cliente_id WHERE c.id = $1 ORDER BY t.realizada_em DESC limit 10",
    values: [id],
  });
